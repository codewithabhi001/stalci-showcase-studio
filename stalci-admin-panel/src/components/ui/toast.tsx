...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="fixed bottom-5 right-5 z-[100] flex w-[min(360px,calc(100vw-2.5rem))] flex-col gap-2.5">
        {toasts.map((t) => {
          ...
          return (
            ...
            >
              <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" style={{ background: s.wash, color: s.color }}>
              ...
              <button onClick={() => dismiss(t.id)} aria-label="Dismiss" className="rounded-md p-1 text-faint hover:bg-canvas hover:text-ink">
