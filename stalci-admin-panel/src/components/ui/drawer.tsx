...
export function Drawer({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-3 sm:p-6 no-print">
      {/* Backdrop */}
      <div
        className="animate-fade-in absolute inset-0 bg-black/40 backdrop-blur-md transition-opacity"
        onClick={onClose}
      />
      ...
      >
        ...
        <header className="flex items-start justify-between gap-4 border-b border-line px-6 py-4.5 shrink-0 bg-surface">
          <div>
            <h2 className="text-base sm:text-lg font-bold text-ink tracking-tight font-display">{title}</h2>
