        ...
        <div className="mx-auto max-w-6xl px-5 lg:px-8">
          ...
          {featured && (
            <Link
              ...
              className="group block rounded-3xl border border-slate-200/90 bg-white p-7 sm:p-10 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-blue-500/40"
            >
              <div className="flex items-center gap-3">
                <span className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-blue-700 bg-blue-50 border border-blue-200 px-3 py-1 rounded-full">
              ...
              <h2 className="mt-4 text-xl sm:text-3xl font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors leading-tight">
                {featured.title}
              </h2>
              ...
              <div className="mt-6 pt-5 border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-4">
                <span className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                  <User className="h-3.5 w-3.5 text-blue-600" /> {featured.author}
                </span>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {rest.map((p: any) => (
              <Link
                ...
                to="/blog/$slug"
                params={{ slug: p.slug }}
                className="group flex flex-col justify-between rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-xl hover:border-blue-500/40"
              >
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-blue-700 bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">
                      {p.category}
                    </span>
                    <span className="text-[11px] text-slate-500 font-mono">{p.readingTime}</span>
                  </div>
                  <h3 className="mt-4 text-base font-bold leading-snug text-slate-900 group-hover:text-blue-600 transition-colors">
                    {p.title}
                  </h3>
