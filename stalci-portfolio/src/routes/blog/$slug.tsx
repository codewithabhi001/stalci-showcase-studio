...
import { getPostBySlug, posts as staticPosts } from "@/lib/blog-data";
    ...
    try {
      ...
      return {
        post: {
          ...
          author: post.author?.name || post.author || "STALCI Engineering",
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-black">
      ...
      <div className="bg-[#000000] text-white pt-32 pb-20 border-b border-white/10">
        <div className="mx-auto max-w-4xl px-5 lg:px-8">
          <Link
            to="/blog"
            className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-emerald-400 hover:text-emerald-300 transition-colors mb-6"
          >
            <ArrowLeft className="h-3.5 w-3.5" /> Back to all articles
          </Link>

          <div className="flex items-center gap-3 mb-4">
            <span className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-3 py-0.5 rounded-full">
              {post.category}
            </span>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-xs text-neutral-400 border-t border-white/10 pt-4">
            <span className="font-semibold text-neutral-200 flex items-center gap-1.5">
              <User className="h-3.5 w-3.5 text-emerald-400" /> {post.author}
            </span>
            <span>•</span>
        ...
        <article className="mx-auto max-w-4xl px-5 lg:px-8">
          <div className="rounded-3xl bg-white border border-zinc-200/90 shadow-sm p-8 sm:p-12 space-y-6">
            {post.body.map((para: string, i: number) => {
              if (para.startsWith("# ")) {
                return (
                  <h1 key={i} className="text-2xl sm:text-3xl font-extrabold text-zinc-950 pt-4 pb-2 border-b border-zinc-200">
                    {para.replace("# ", "")}
                  </h1>
                );
              }
              if (para.startsWith("## ")) {
                return (
                  <h2 key={i} className="text-xl sm:text-2xl font-bold text-zinc-950 pt-4 pb-1 border-b border-zinc-100">
                    {para.replace("## ", "")}
                  </h2>
                );
              }
              if (para.startsWith("### ")) {
                return (
                  <h3 key={i} className="text-lg font-bold text-zinc-900 pt-2">
                    {para.replace("### ", "")}
                  </h3>
                );
              }
            ...
            <div className="mt-12 rounded-2xl bg-gradient-to-br from-black to-[#0A0A0A] border border-white/10 text-white p-7 sm:p-9 shadow-lg">
              ...
              <Link
                to="/contact"
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white hover:bg-zinc-200 px-5 py-2.5 text-xs font-bold text-black transition-all shadow-md"
              >
          ...
          <div className="mt-16">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {related.map((p) => (
                ...
                >
                  <span className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-emerald-600">
                    {p.category}
                  </span>
