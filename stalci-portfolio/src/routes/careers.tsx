...
function Careers() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900">
      ...
      {/* Header Banner */}
      <div className="bg-[#080B12] text-white pt-28 pb-16 border-b border-white/10">
        <div className="mx-auto max-w-5xl px-5 lg:px-8">
          <span className="inline-block text-[10px] font-mono font-bold uppercase tracking-widest text-slate-400 mb-3">
      ...
      <main className="py-14 sm:py-20">
        <div className="mx-auto max-w-5xl px-5 lg:px-8">
          ...
          <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4 mb-12">
            {benefits.map((b) => (
              <div key={b.title} className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs">
          ...
          <div className="mb-6">
            <h2 className="text-lg font-bold text-slate-950">Open Positions</h2>
            <p className="text-xs text-slate-500 mt-0.5">Live active roles managed by STALCI Talent Operations.</p>
          </div>
          ...
          <div className="space-y-3">
            {jobsList.map((j: any) => {
              ...
              return (
                <div
                  key={j.id}
                  className="rounded-xl border border-slate-200 bg-white shadow-2xs overflow-hidden transition-all hover:border-slate-400"
                >
                  <div
                  ...
                  >
                    <div>
                      <h3 className="text-sm sm:text-base font-bold text-slate-950">{j.title}</h3>
                      <p className="mt-1 text-xs text-slate-600 max-w-xl">{j.description}</p>
                    </div>

                    <div className="flex flex-wrap items-center gap-2 shrink-0 text-[11px] font-medium text-slate-600">
                      <span className="inline-flex items-center gap-1 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-md">
                        <MapPin className="h-3 w-3 text-slate-500" /> {j.location}
                      </span>
                      <span className="inline-flex items-center gap-1 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-md">
                        <Clock className="h-3 w-3 text-slate-500" /> {j.type}
                      </span>
                      <ChevronDown
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 pt-2 border-t border-slate-100 bg-[#F8FAFC]">
                      <h4 className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-600 mt-2 mb-2">
                        Key Qualifications
                      </h4>
          ...
          {/* Open Inquiries Card */}
          <div className="mt-12 rounded-2xl bg-[#080B12] text-white p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 border border-white/10">
            <div>
            ...
            <a
              ...
              className="shrink-0 rounded-full bg-white hover:bg-slate-200 text-slate-950 font-semibold px-5 py-2 text-xs transition-colors"
            >
          ...
          >
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmitApplication} className="space-y-3.5">
                <div>
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500">Application</span>
                  <h3 className="text-base font-bold text-slate-950 mt-0.5">{applyingJob.title}</h3>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-slate-50 p-2 text-xs text-slate-900 outline-none focus:outline-slate-900"
                  />
                </div>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full rounded-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-2.5 text-xs transition-colors flex items-center justify-center gap-2 cursor-pointer"
                  >
