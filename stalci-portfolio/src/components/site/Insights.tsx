...
export function Insights() {
  ...
  return (
    <section id="blog" className="relative bg-white py-14 sm:py-20 text-slate-900 border-t border-slate-200/90">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-3">
          {latest.map((p) => (
            <Link
              ...
              to="/blog/$slug"
              params={{ slug: p.slug }}
              className="flex flex-col rounded-xl border border-slate-200/90 bg-white p-5 transition-all duration-200 hover:border-blue-500/40 hover:shadow-md group shadow-sm"
            >
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-blue-600">
                {p.category}
              </span>
              <h3 className="mt-2 text-sm font-bold leading-snug text-slate-900 group-hover:text-blue-600 transition-colors">
                {p.title}
              </h3>
              <p className="mt-1.5 flex-1 text-xs leading-relaxed text-slate-600 line-clamp-3">{p.excerpt}</p>
              <span className="mt-3 text-[11px] text-slate-500 font-mono">{p.readingTime}</span>
            </Link>
          ))}
        </div>
