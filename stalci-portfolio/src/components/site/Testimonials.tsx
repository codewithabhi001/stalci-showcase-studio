...
export function Testimonials() {
  ...
  return (
    <section className="relative overflow-hidden bg-white py-20 sm:py-28 text-black border-t border-zinc-200/90">
      <div className="relative mx-auto max-w-6xl px-5 lg:px-8">
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <button
            ...
            className="self-start md:self-auto shrink-0 inline-flex items-center gap-2 rounded-full bg-black hover:bg-zinc-800 text-white text-xs font-semibold px-4 py-2 transition-colors cursor-pointer"
          >
        ...
        <div ref={staggerRef} className="mt-12 grid gap-4 md:grid-cols-3">
          {testimonials.map((t, idx) => (
            <motion.div
              key={idx}
              className="flex flex-col justify-between rounded-xl border border-zinc-200 bg-[#F8FAFC] p-5 sm:p-6 transition-all duration-200 hover:border-zinc-400 hover:shadow-xs"
            >
              <div>
                <div className="flex gap-1 text-zinc-400 mb-3">
                  {Array.from({ length: t.rating || 5 }).map((_, i) => (
                    <Star key={i} className="h-3.5 w-3.5 fill-zinc-800 text-zinc-800" />
                  ))}
                </div>
                <Quote className="h-6 w-6 text-zinc-300 mb-2" />
                <p className="text-xs sm:text-sm leading-relaxed text-zinc-700 italic">"{t.quote}"</p>
              </div>

              <div className="mt-5 pt-4 border-t border-zinc-200">
                <p className="text-xs sm:text-sm font-bold text-zinc-950">{t.name}</p>
          ...
          >
            ...
            {submitted ? (
              <div className="text-center py-6">
                ...
                <h3 className="text-lg font-bold text-zinc-950">Thank You for Your Feedback!</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-zinc-500">Client Review</span>
                  <h3 className="text-base font-bold text-zinc-950 mt-0.5">Submit Project Feedback</h3>
                </div>

                <div>
                  <label className="text-xs font-semibold text-zinc-950 block mb-1">Rating</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-950 block mb-1">Your Name & Company</label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="e.g. Alex Carter, CTO at Acorn Corp"
                    className="w-full rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-xs text-zinc-900 focus:outline-black"
                  />
                </div>

                <div>
                  <label className="text-xs font-semibold text-zinc-950 block mb-1">Feedback</label>
                  <textarea
                    rows={3}
                    value={comments}
                ...
                <button
                  ...
                  className="w-full rounded-full bg-black text-white text-xs font-semibold py-2.5 hover:bg-zinc-800 transition-colors cursor-pointer"
                >
